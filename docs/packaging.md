...
3. 导出 Excel 明细报告和 HTML 摘要报告，确认 `summary`、`high_risk_findings`、`findings`、`errors` 等工作表可打开，HTML 可离线阅读。
...
该脚本默认扫描 `sample_data/files/`，并将 Excel 明细报告和 HTML 摘要报告输出到本地 `audit_reports/`。
